...
class RGATLayer(nn.Module):
    def __init__(self, dim, edge_type_num, alpha, dropout=0.):
        ...
        self.training_edges = list(range(1, self.edge_type_num+1)) # 1~6
    ...
    def forward(self, hidden, adj, stage='training'):
        ...
        alpha = self.dropout_atten(alpha)
...
class GATLayer(nn.Module):
    def __init__(self, dim, alpha, dropout=0., bias=False):
        super(GATLayer, self).__init__()
        self.dim = dim
        self.dropout = dropout

        self.W = nn.Parameter(torch.FloatTensor(dim, dim))
        self.a_params = nn.Parameter(torch.Tensor(self.dim, 1))
        self.bias = bias
    ...
    def forward(self, hidden, adj):
        ...
        alpha = -9e15 * torch.ones(batch_size, N, N).cuda() # [B L L]
        e = torch.matmul(a_input, self.a_params)
        e = self.leakyrelu(e).squeeze(-1).view(batch_size, N, N)
        alpha = torch.where(adj.eq(1), e, alpha)

        alpha = torch.softmax(alpha, dim=-1)
        output = torch.matmul(alpha, hidden)

        return output, e # [B L D]
