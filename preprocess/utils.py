...
def extract_raw_image_feature(config, device='cuda', batch_size=218, image_dir=''):
    print("utils.py: extracting image feature from {}".format(image_dir))
    
    # 加载ResNet-50模型，将其设为评估模式
    model_name = 'resnet50' if config['image_extractor'] == 'resnet' else config['image_extractor']
    module = importlib.import_module('torchvision.models')  # 导入models模块
    # 获取模型函数/类并实例化
    ModelClass = getattr(module, model_name)
    model = ModelClass(pretrained=True).to(device)
    print("using the extraction model of:", type(model).__name__)
...
def extract_raw_text_feature(config, title_corpus, asin_list, tokenizer, model, batch_size=8):
    if config['text_extractor'] == 't5':
        from transformers import T5EncoderModel
        model = T5EncoderModel.from_pretrained(config['from_pretrained_name']).cuda()
    elif config['text_extractor'] == 't5large':
        from transformers import T5EncoderModel
        model = T5EncoderModel.from_pretrained(config['from_pretrained_name']).cuda()
    elif config['text_extractor'] == 'opt125m':
        from transformers import OPTModel
        model = OPTModel.from_pretrained(config['from_pretrained_name']).cuda()
    elif config['text_extractor'] == 'opt350m':
        from transformers import OPTModel
        model = OPTModel.from_pretrained(config['from_pretrained_name']).cuda()
    elif config['text_extractor'] == 'chatglm':
        from transformers import AutoModel
        model = AutoModel.from_pretrained(config['from_pretrained_name'], trust_remote_code=True).half().cuda()
        batch_size = 1
    print("using the extraction model of:", type(model).__name__)
    ...
    for batch_idx in tqdm(range(batch_num)):
        ...
        inputs = tokenizer(processing_text, return_tensors="pt", padding=True, truncation=True).to(model.device)  # Batch size 1
        if config['text_extractor'] == 'chatglm':
            outputs = model.transformer(**inputs, output_hidden_states=True)
            hidden_states = outputs.hidden_states
            last_hidden_states = hidden_states[-1]
            sentence_embedding = last_hidden_states.mean(dim=0)
        else:
            outputs = model(**inputs, output_hidden_states=True)
            last_hidden_states = outputs.last_hidden_state
            sentence_embedding = last_hidden_states.mean(dim=1)
        np_sentence_embedding = sentence_embedding.detach().numpy() if model.device == 'cpu' else sentence_embedding.detach().cpu().numpy()
        if len(features_array) == 0:
            features_array = np_sentence_embedding
        else:
            features_array = np.concatenate((features_array, np_sentence_embedding), axis=0)
